...
export const privacyPolicyContent: Record<Language, PrivacyPolicyContent> = {
  zh: {
    ...
    sections: [
      {
        title: '1. 信息处理范围',
        content: `本应用（"费曼读书助手"）尊重您的隐私。登录账号后，以下用户主动产生的学习数据会按账号保存到云端：
...
平台不会收集手机号、邮箱、精确位置或与阅读无关的敏感信息。使用 AI 功能时，完成当前任务所需的相关输入会发送给所选的大模型服务。`
      },
      {
        title: '2. 数据存储',
        content: `本应用使用账号对应的云端空间保存登录后的学习数据，并仅将浏览器 IndexedDB 用于旧版本历史数据迁移：
...
尚未迁移或尚未导出备份的数据无法代为恢复。云端数据与登录账号绑定，可在账号中心查看、导出和删除。`
      },
      {
        title: '3. API Key 使用',
...
使用 AI 功能时，可能发送的内容包括书名、作者、简介、阶段学习输入、教学模拟内容、角色问答及用于生成推荐的相关学习内容。登录后上传文档的解析原文保存在账号对应的云端空间；调用 AI 时，较短文档可能发送完整原文，较长文档会从完整原文中检索并发送与当前任务相关且覆盖不同位置的片段。请勿在这些内容中填写不希望提供给所选 AI 服务的个人敏感信息。
...
应用仅负责传输当前任务所需的数据，不会将无关内容发送给大模型服务。`
      },
      {
        title: '7. Cookie 使用',
    ...
    sections: [
      {
        ...
        content: `This app ("Feynman Reader") respects your privacy. After sign-in, user-created learning data is stored in your account cloud space, including:
...
The platform does not collect phone numbers, email addresses, precise location, or sensitive data unrelated to reading. AI requests are proxied by the application server and include only input needed for the current AI task.`
      },
      {
        title: '2. Data Storage',
        content: `This app uses account cloud storage for signed-in learning data and uses browser IndexedDB only to migrate history from older versions:
...
Unmigrated or unexported data cannot be recovered for you. Cloud data is scoped to your signed-in account and can be viewed, exported, or deleted in Account Center.`
      },
      {
...
AI features may send the book title, author, description, learning-phase input, teaching-practice content, persona Q&A, and related learning content used to generate recommendations. After sign-in, the complete parsed source is stored in your account cloud space. Short documents may be sent in full for an AI request; for longer documents, task-relevant excerpts are retrieved from the complete source. Do not include personal sensitive information that you do not want to provide to the selected AI service.
...
• Account cloud data syncs automatically; import and export tools are available whenever you need them
...
The selected model service processes related data under its terms of service and privacy policy.`
      },
      {
