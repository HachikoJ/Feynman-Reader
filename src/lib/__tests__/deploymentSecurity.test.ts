...
describe('production security header deployment', () => {
  ...
  it('publishes a standalone Node service without deleting chunks used by open pages', () => {
    ...
    expect(deployScript).toContain('pm2 start')
  ...
  it('retires old product aliases before they can reach configuration screens', () => {
    ...
    expect(productNginx).toContain('location ^~ /feynmanreader/ { return 410; }')
    expect(tokendanceSource).toContain("TOKENDANCE_APP_URL = 'https://www.deline.top'")
    expect(tokendanceSource).toContain('TOKENDANCE_REQUEST_APP_URL = TOKENDANCE_APP_URL')
    expect(readFileSync(join(process.cwd(), 'src/app/api/ai/chat/completions/route.ts'), 'utf8')).toContain('TOKENDANCE_REQUEST_APP_URL')
