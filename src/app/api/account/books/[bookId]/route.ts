...
export async function GET(request: Request, context: Context): Promise<NextResponse> {
  try {
    ...
    if (!store.getBook) return NextResponse.json({ error: '数据库尚未启用书籍读取。' }, { status: 501 })
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : '读取书籍失败。' }, { status: 500 })
  }
}
