...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const payload = await request.json() as { settings?: unknown; books?: unknown[]; aiUsageRecords?: unknown[]; bookLists?: unknown[]; bookRelations?: unknown[]; version?: unknown }
    if (!Array.isArray(payload.books) || payload.books.length > 10000 || !Number.isInteger(payload.version)) return NextResponse.json({ error: '导入文件格式无效。' }, { status: 400 })
    const store = getPersistence()
    if (!store.importUserData) return NextResponse.json({ error: '数据库尚未启用数据导入适配器。' }, { status: 501 })
    const result = await store.importUserData(userId, { ...payload, settings: undefined })
