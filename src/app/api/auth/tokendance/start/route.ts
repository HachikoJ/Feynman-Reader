...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const state = createOAuthState({ nonce, callback, issuedAt: Date.now() })
  ...
  response.headers.append('Set-Cookie', `feynman_watcha_pkce=${encodeURIComponent(`${nonce}.${verifier}`)}; Path=/api/auth/tokendance/callback; HttpOnly; Secure; SameSite=Lax; Max-Age=600`)
  return response
}
