...
export default function UndoRedoControls({ lang }: Props) {
  ...
  const handleUndo = async () => {
    ...
    try {
      ...
      if (!succeeded) {
        showStatus(lang === 'zh' ? '撤销未能保存，请检查登录和网络后重试。' : 'Undo could not be saved. Check sign-in and network, then try again.')
        return
      }
      setShowHint(true)
  ...
  const handleRedo = async () => {
    ...
    try {
      ...
      if (!succeeded) {
        showStatus(lang === 'zh' ? '重做未能保存，请检查登录和网络后重试。' : 'Redo could not be saved. Check sign-in and network, then try again.')
        return
      }
      setShowHint(true)
      setTimeout(() => setShowHint(false), 2000)
        ...
        try {
          if (!await undoRedoManager.undo()) {
            window.dispatchEvent(new CustomEvent('undo-redo-status', {
              detail: { message: lang === 'zh' ? '撤销未能保存，请检查登录和网络后重试。' : 'Undo could not be saved. Check sign-in and network, then try again.' }
            }))
          }
        } finally {
      ...
      if ((e.ctrlKey || e.metaKey) && (e.key === 'y' || (e.key === 'z' && e.shiftKey))) {
        ...
        try {
          if (!await undoRedoManager.redo()) {
            window.dispatchEvent(new CustomEvent('undo-redo-status', {
              detail: { message: lang === 'zh' ? '重做未能保存，请检查登录和网络后重试。' : 'Redo could not be saved. Check sign-in and network, then try again.' }
            }))
          }
        } finally {
          shortcutInFlight = false
