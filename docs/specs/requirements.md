...
### 1.1 产品定位
费曼读书助手是一款基于费曼学习法的智能阅读辅助工具，通过"以教代学"的方式帮助用户深度理解书籍内容。

### 1.2 核心价值主张
- **深度理解**: 通过6个阶段的系统化分析，全方位理解一本书
- **主动学习**: 通过教学模拟和角色问答，主动发现理解盲点
- **AI辅助**: 利用 DeepSeek V4 Flash 进行智能分析、评估和反馈
...
- ✅ AI分析时使用文档内容
- ✅ 分析结果更准确相关
- ✅ 短文使用完整上下文，长文从完整原文检索相关及分布式片段
- ✅ 网关返回上下文超限时自动逐级缩小文档上下文；全部失败才就地提示且保留已有内容
...
- **全文检索上下文**: 原文完整保存在本地；AI分析时按任务控制上下文长度并覆盖全文不同位置
- **上下文预算**: 按 100 万 Token 窗口预留约 20 万 Tokens，用于系统提示词、用户输入、结构化包装、输出及分词误差；中文文档上下文上限约 60 万字符
- **超限降级**: 不依赖模型自动截断；阶段学习和教学评估遇到上下文超限时，按 60万、45万、30万、15万字符逐级缩小
...
费曼读书助手是一款功能完善、设计精良的深度阅读工具。通过系统化的六阶段学习、严格的费曼实践评估、智能的AI辅助，帮助用户真正深入理解书籍内容。
