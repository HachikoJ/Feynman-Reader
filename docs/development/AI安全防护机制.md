...
- 提示词将任务范围限定在书籍学习相关内容
- 对常见无关或越界请求进行拒绝或引导
- 模型仍可能受提示注入和输出波动影响，不能把角色约束当作绝对安全保证


### 2. 能力边界
- 系统提示词位于公开客户端源码中，不属于秘密，也不能作为安全边界
- 提示词可以约束正常对话方向，但无法保证模型永远拒绝提示注入
- 评分结果仅用于学习反馈，不能用于授权、付费、数据删除等安全决策
- 角色与输出结构仍可能受模型波动影响，需要由本地代码做格式与范围校验

### 3. 用户体验
- 正常用户不受影响
- 对常见越界请求进行提醒或拒绝，降低无关输出概率
- 保持专业和友好的交互
...
- 公开前端中的 Prompt、阈值和业务规则都可以被查看，不应放置密钥、万能码或真正需要保密的规则。
- 仅靠 Prompt 无法彻底防止注入攻击；若未来涉及账号、计费、授权或敏感数据，应迁移到服务端执行并校验。
- AI 输出必须视为不可信输入，渲染、链接跳转和结构化数据使用前仍需由应用代码限制。
...
当前防护通过任务限定、输入边界、结构校验、安全渲染和本地业务规则，降低无关输出、提示注入与异常 AI 响应影响产品状态的概率。它不承诺提示词保密，也不能保证模型行为绝对稳定；公开前端中的规则都应视为可见信息。

真正影响授权、计费、数据删除或账号权限的能力，未来必须放到可信服务端执行。现阶段的重点是确保 AI 输出只作为不可信的学习建议输入，由客户端校验后再展示或写入学习记录，并持续用回归测试覆盖已知边界。
