...
每次上线前，先为「当时正在运行的提交」建立一个只读快照标签，标签名形如 `deploy-v0.2.7-f248026`，并发布同名 Release，附 `feynman-reader-deploy-<日期>-v<版本>-<短提交>.tar.gz`、`SHA256SUMS` 和 `release-manifest.json`。快照标签不移动、不覆盖；如果线上提交还没有功能版本标签，也必须有对应的快照标签，否则回滚只能依赖服务器上的发布目录。

功能版本标签（`vX.Y.Z`）描述产品版本，部署快照标签描述「线上实际运行的提交」，两者同时保留：即线上运行的提交是 v0.2.7 之后的一次修复时，先建 `deploy-v0.2.7-<短提交>` 快照，再发布新功能版本。
